<template>
<<<<<<< HEAD
  <div class="top">
    <div class="top-header">
      <div class="logo">
        <img src="../../../../public/img/logo.png"/>
        <input type="text" placeholder="搜索" />
      </div>
      <img src="../../../../public/img/tv.png"/>
    </div>
  </div>
</template>
<script>
export default {
  name: "Header"
};
</script>
<style lang="scss">
.top {
  width: 100%;
  height: 0.417rem;
  display: flex;
  position: fixed;
  justify-content: center;
  box-sizing: border-box;
  background-color: #fff;
  z-index: 10;

  .top-header {
    display: flex;
    align-items: center;
    width: 2.875rem;
    height: 0.417rem;
    justify-content: space-between;

      > img {
        width: 0.167rem;
        height: 0.158rem;
      }

      .logo {
         img{
            width: 0.45rem;
            height: 0.217rem;
            float: left;
            margin-right: 0.125rem;
        }
        input {
            float: left;
            display: flex;
            width: 0.833rem;
            height: 0.25rem;
            background: #f7f7f7 url("../../../../public/img/big.png") no-repeat 8px center;
            background-size: 18px;
            border: 1px solid #dfdfdf;
            border-radius: 0.125rem;
            padding-left: 0.25rem;
            box-sizing: border-box;
            outline: none;
        }
      }
  }
}

=======
    <div id="header">
        <div class="top">
            <div class="top-header">
                <div class="logo">
                    <img src="data:image/png;base64," alt="">
                    <!-- @public/img/logo.png -->
                    <input type="text" placeholder="搜索">
                </div>
                <img src="data:image/png;base64," alt="">
                <!-- @public/img/tv.png -->
            </div>
        </div>
    </div>
</template>

<script>
export default {
    name:"Header",
    props:{
        title:{
            type:String,
            default:"豆瓣"
        }
    }
}
</script>

<style>
    #header{
        position: relative;
        width: 100%;
        height: .417rem;
        display: flex;
        justify-content: center;
        align-items: center;
        box-shadow: 0 1px 4px rgba(0,0,0,0.05);
        background: #fff;
        color: #fff;
    }
    .top{
        width: 100%;
        height: .417rem;
        display: flex;
        display: fixed;
        justify-content: center;
        box-sizing: border-box;
    }
    .top-header{
        display: flex;
        align-items: center;
        width: 2.875rem;
        height: .417rem;
        justify-content: space-between;
    }
    .logo>img:nth-of-type(1){
        width: .45rem;
        height: .217rem;
        float: left;
        margin-right: .125rem;
    }
    .logo input{
        float: left;
        display: flex;
        width: .833rem;
        height: .25rem;
        background: #f7f7f7 url("data:image/png;base64,") no-repeat 8px center;
        background-size: 18px;
        border: 1px solid #dfdfdf;
        border-radius: .125rem;
        padding-left: .25rem;
        box-sizing: border-box;
    }
    .top-header>img{
        width: .167rem;
        height: .158rem;
    }
>>>>>>> 43315f135e2740ba6028db3c4fcfafaf34d718b7
</style>